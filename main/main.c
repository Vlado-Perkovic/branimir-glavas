...
#include "common_bme69x/common.h"
...
void app_main() {
    ...
    // struct bme69x_data *data = NULL;
    struct bme69x_data data[3];
    uint8_t n_fields;
    bme69x_get_data(BME69X_SEQUENTIAL_MODE, data, &n_fields, bme69x_handle);
    for (uint8_t i = 0; i < 3; i++)
    {
            printf("%f, %lu, %lu, %lu, 0x%x, %d, %d\n",
                   (data[i].temperature),
